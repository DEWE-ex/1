...
import { BannedUserError, createSharePost } from "@/lib/shares";
...
export default function ShareWriteModal({
  ...
  const { playerId, displayName, photoURL, authMode } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!content.trim() || !playerId || authMode === "none") return;
    setSubmitting(true);
    setError(null);
    try {
      ...
      if (err instanceof BannedUserError) {
        setError(
          "Tài khoản của bạn đã bị cấm đăng bài. Liên hệ admin để biết thêm."
        );
      } else {
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-2.5">
          ...
          {error && (
            <p className="rounded-lg bg-red-500/10 px-2 py-1.5 text-center text-xs font-medium text-red-600 dark:text-red-300">
              {error}
            </p>
          )}
          <button
