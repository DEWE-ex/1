...
function shouldInitiateMatch(playerId: string, opponentId: string): boolean {
  return playerId.localeCompare(opponentId) < 0;
}
...
async function joinMatchmakingInternal(
  ...
  if (!shouldInitiateMatch(playerId, opponentId)) {
    return { status: "waiting" };
  }
