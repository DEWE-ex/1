...
export async function startNextRound(roomCode: string): Promise<void> {
  ...
  const retiredCardIds = room.retiredCardIds ?? [];
  const questions = (await getApprovedQuestions(category)).filter(
    (q) => !retiredCardIds.includes(q.id),
  );
...
export async function handleCardClick(
  ...
  await runTransaction(roomRef, (room: Room | null) => {
    ...
    const gameOver = newHostScore >= maxScore || newGuestScore >= maxScore;
