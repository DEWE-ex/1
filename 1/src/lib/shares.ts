...
export class BannedUserError extends Error {
  constructor() {
    super("Tài khoản của bạn đã bị cấm đăng bài");
...
export async function createSharePost(
  ...
  if (await isUserBanned(authorId)) {
...
export async function reportSharePost(
  postId: string,
  reporterId: string,
  reporterName: string,
  reason: string,
  ...
  const db = getDb();
  const reportRef = ref(db, `shareReports/${postId}/${reporterId}`);
  ...
  await set(reportRef, {
    ...
    reporterId,
    reporterName,
    reason: reason.trim() || "Nội dung không phù hợp",
    createdAt: Date.now(),
    postAuthorName: post?.authorName ?? null,
    postPreview: post?.content?.slice(0, 120) ?? null,
  });
}
...
export async function hasReportedPost(
  ...
  try {
    const db = getDb();
    const snap = await get(ref(db, `shareReports/${postId}/${userId}`));
    return snap.exists();
  } catch {
    return false;
  }
